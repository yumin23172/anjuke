...
#跨站请求伪造
    ...
    def put(self):
        ...
        data = {
            ...
            "rentType": "1",                                                     #1整租2合租（rentType）默认1
            "community58": item["xiaoquname"],                                   #小区名（community_unite）
            'wuba_comm_id': self.xq,                                             #小区id
            'zone58': self.r,
            'biz58': self.m,
            'address58': self.address,
            'room': item["shi"],                                                 #室（room）
            'hall': item["ting"],                                                #厅（hall）
            'bathroom': item["wei"],                                             #卫（bathroom）
            'floor': item["louceng"],                                            #楼层（floor）
            'allFloor': item["zonglouceng"],                                     #总楼层（allFloor）
            'lift': '1',                                                         #是1否2有有电梯（lift）
            'housetype': item["chanquanleixing"],                                #房屋类型（housetype）
            'housefit':  item["zhuangxiu"],                                      #装修情况（housefit）
            'exposure': item["chaoxiang"],                                       #朝向（exposure）
            'roomarea': item["mianji"],                                          #面积（roomarea）
            'rentprice': item["price"],                                          #价格（rentprice）
           'paymode': item["fukuanfangshi"],                                     #付款方式（paymode）
        ...
        print("*" * 80)
        res = requests.post(
            'https://vip.anjuke.com/house/publish/rent/?chooseWeb%5B%5D=2', headers=headers_1,
        ...
        if flag_1:
            if flag_1[0] == "发布成功":
                ...
                response = requests.get(url="https://vip.anjuke.com/zf58/kcfy58",
...
"fukuanfangshi":"2",
"title":"你据格斤复怀旧格方法给寒计较斤斤计较斤斤计较斤斤计较",
